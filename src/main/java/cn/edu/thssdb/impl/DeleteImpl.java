...
public class DeleteImpl {
  public static void handleDeletePlan(DeletePlan deletePlan, Database currentDataBase, Long currentSessionId) throws NoCurrentDatabaseException, DeleteWithoutWhereException {
      ...
      if (!deletePlan.getWhereCond().hasChild) {
        ...
        if (singleConditionPlan.expr1.type == ComparerType.COLUMN && singleConditionPlan.expr2.type != ComparerType.COLUMN) {
          ...
          if (currentTable.Column2Index(attrName) == currentTable.primaryIndex) {
            currentTable.removePrimaryKey(new Entry((Comparable) singleConditionPlan.expr2.getValue()));
            return;
          }
        }
      }
